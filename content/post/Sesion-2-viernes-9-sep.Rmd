...
h1<-ggplot(peso, aes(x=pesoh)) + geom_histogram(fill="white", colour="blue") +
...
Nota: Observe que el eje horizontal esta exprezado en términos de un valor exprezado en el numero de desviaciones estándar desde la media, representada con el valor de cero. Estos valores se denominan **valores Z  o Z-scores**. 
...
así por ejemplo la gráfica indica intervalos de[-4,4], que equivalen la dsitribución de datos por encima o superiores hasta Cuatro desviaciones estándard de la media y datos inferiores en est misma distancia respecto del valor promedio.
...
[Ver documentación](https://www.rdocumentation.org/packages/EnvStats/versions/2.3.1/topics/qqPlot)
