...
Con los datos sobre temperatura disponibles.

Estime la trayectoria de precipitación media  y el rango intercuartil.

Representación en line plot. 
...
t_url<-("https://github.com/JoseLuisManzanaresRivera/intersemestral-2021/blob/main/content/post/temperatura_ags.rds?raw=true")

t<-read_rds_from_github(t_url)%>%
  rename(grados=promedio)
...
saveRDS(precip, "precip_tijuana.csv")

precip<-group_by(precip,año,mes)%>%
  summarise(mesp=sum(PRECIPmm))

anual<-group_by(precip,año)%>%
  summarise(t=sum(mesp))

summary(anual)

saveRDS(mesavg,"temperatura_ags.rds")


mesavg<-mutate(mesavg,año=as.numeric(año))
febrero<-mutate(precip,año=as.numeric(año))
...
plt<-ggplot(febrero, aes(x=año, y=mesp))+
geom_line(size=.3, colour="gray")+
geom_smooth()+
xlab("Año")+
ylab("Precipitación  promedio. 1969-2012 (mm)")+
theme_light()+
geom_hline(yintercept = 21, size=.5, linetype="dashed",color='red')+
geom_hline(yintercept = 84.38, size=.5, linetype="dashed",color='darkblue')
