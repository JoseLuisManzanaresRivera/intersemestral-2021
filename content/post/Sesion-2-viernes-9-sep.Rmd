...
Nota: Observe que el eje horizontal esta exprezado en términos de un valor que mide  el número de desviaciones estándar desde la media, representada con el valor de cero. Estos valores se denominan **valores Z  o Z-scores**. 

Estos se denominan **Z scores** o valores estandar por que permiten un criterio de comparación entre la distribución de varibles  y nos indican la distancia desde la media 
exprezada en unidades de desviación estandar, ($\sigma$), 
así por ejemplo la gráfica indica intervalos de[-4,4], que equivalen a la dsitribución de datos por encima o superiores hasta Cuatro desviaciones estándard de la media y datos inferiores en esta misma distancia respecto del valor promedio.


**Note** que un **patrón concavo** de los datos respecto a la linea de 45° que representa una distribución teórica simétrica (normal), es indicativo de sesgo positivo: ej. la cola de la dsitribución de los datos se extiende  hacia el cuadrante positivo.
...
Determine si la varaiable talla (medida en centimetros) de la base de datos correspondeinte a caracteristicas de recien nacidos, exhibe una distribución simetrica (normal) de los datos.
...
Nota: El teorema de límite central nos indica que independientente de la distribución de un conjunto de datos, **la distribución muestral** tiende a comportarse acorde a la distribución normal, si la muestra es lo suficientemente grande (n>30). 
...
[Link descarga datos ejemplo 2](https://drive.google.com/file/d/1g9IJd84y8Qa1V6pfg8SyB01rvyB0o2R5/view?usp=sharing)
...
bc_temp<-read.csv("baja_temp.csv")
...
boxplot_2<-ggplot(bc_temp, aes(x=fct_inorder(meses), y=temp)) + geom_boxplot(aes(fill=maxmin))+
xlab("Mes")+
...
[Link descarga datos práctica]("https://drive.google.com/file/d/1ZiBDa7FGMc9_fdGCIV76AzNVQ0LMey7m/view?usp=sharing")

```{r}
## Paso 1 Read datos 

practica<-read.csv("tij_practica.csv")
...
Note que se tiene una linea de tendencia, esta resulta de la aplicación del parámetro **geom_smooth** este actua como un filtro que permite observar la tendencia de la temperatura 
a lo largo de la serie de tiempo. 

Su aplicación *"suaviza"* el comportamiento de la variable en niveles, (línea en gris). En el *background*  el parámetro **geom_smooth** realiza una **regresión lineal por el método de mínimos cuadrados ordinarios** (un tema que desarrollaremos a detalle en la sección siguiente), usando la especificación **LOESS** (locally estimated scatterplot smoothing). 
