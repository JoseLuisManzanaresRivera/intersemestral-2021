...
exa<-read.csv("C:/Users/josel/Documents/example.csv" )%>%select(-c("X", "X.1","X.2","X.3"))%>%na.omit()%>%
rename(participantes.f=No.of.ear.heads..f.,ingreso.x=Mid.x )%>%
summarize(sumalogs=sum(f.log.x), n=sum(participantes.f), div=sumalogs/n,gmean=10^(div), eee=exp(div))
...
## Tal como el algoritmo de estimación de media geométrica indica, obtenemos la media aritmetrica de los logaritmos y posteriormente reexpresamos el indicador de a niveles es decir no expresados en logaritmos. Note que el calculo usa la base de logaritmo natural que es tomando como base el número e=2.718281828....
